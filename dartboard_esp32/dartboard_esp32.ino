 ...
 *
 * pyDarts server is discovered automatically via mDNS (_pydarts._tcp).
 * WiFi credentials are configured via a captive portal (no reflash needed).
 *
 * Libraries required (Arduino Library Manager):
 *   - WiFiManager  by tzapu
 *   - WebSockets   by Markus Sattler
 *   - ArduinoJson  by Benoit Blanchon
 * Built-in (ESP32 Arduino framework):
 *   - ESPmDNS
 */
...
// ─── mDNS discovery ──────────────────────────────────────────────────────────
bool discoverServer() {
  Serial.println("[mDNS] Searching for pyDarts server...");
  int n = MDNS.queryService("pydarts", "tcp");
  if (n > 0) {
    wsHost = MDNS.IP(0).toString();
    wsPort = MDNS.port(0);
    Serial.printf("[mDNS] Found pyDarts at %s:%d\n", wsHost.c_str(), wsPort);
    return true;
  }
  Serial.println("[mDNS] Not found.");
  return false;
}
...
void connectWebSocket() {
  ...
  if (!found) {
    // Fallback to hardcoded IP in config.h (if defined)
    #ifdef WS_HOST_FALLBACK
      wsHost = WS_HOST_FALLBACK;
      wsPort = WS_PORT;
      Serial.printf("[WS] Fallback to config: %s:%d\n", wsHost.c_str(), wsPort);
    #else
      Serial.println("[WS] No server found. Retrying in 10s...");
      delay(10000);
      return;
    #endif
  }
  wsClient.begin(wsHost, wsPort, WS_PATH);
...
const char* scanMatrix() {
  for (int m = 0; m < NUM_MASTERS; m++) {
    digitalWrite(masterPins[m], LOW);

    for (int s = 0; s < NUM_SLAVES; s++) {
      if (digitalRead(slavePins[s]) == LOW) {
        for (int i = 0; i < NUM_SEGMENTS; i++) {
          if (SEGMENTS[i].master == masterPins[m] &&
              SEGMENTS[i].slave  == slavePins[s]) {
            digitalWrite(masterPins[m], HIGH);
...
// Hold BTN_OPTIONS (master=GPIO2, slave=GPIO22) at boot to reset WiFi config
bool isConfigButtonHeld() {
  pinMode(2,  OUTPUT); digitalWrite(2, LOW);
  pinMode(22, INPUT_PULLUP);
  delay(50);
  bool held = (digitalRead(22) == LOW);
  pinMode(2, OUTPUT); digitalWrite(2, HIGH);   // restore
  return held;
}
...
void setup() {
  ...
  if (isConfigButtonHeld()) {
    Serial.println("[WiFi] BTN_OPTIONS held — resetting WiFi config...");
    wm.resetSettings();
  }
  // Opens "pyDarts-Setup" AP if no stored credentials
  // Connect to 192.168.4.1 in a browser to configure
  wm.setConfigPortalTimeout(180);   // 3 min timeout, then retry
  if (!wm.autoConnect("pyDarts-Setup")) {
  ...
  // Start mDNS
  if (!MDNS.begin("esp32-darts")) {
    Serial.println("[mDNS] Failed to start");
  }

  // Discover and connect to pyDarts
  connectWebSocket();
}
...
void loop() {
  ...
  // Re-run mDNS discovery every 30s when disconnected
  if (!wsConnected && millis() - lastDiscovery > 30000) {
